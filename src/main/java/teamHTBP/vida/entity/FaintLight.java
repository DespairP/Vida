...
public class FaintLight extends Entity implements IEntityAdditionalSpawnData {

    private static final Logger LOGGER = LogManager.getLogger();
    private static final EntityDataAccessor<String> TYPE = SynchedEntityData.defineId(FaintLight.class, EntityDataSerializers.STRING);
    private IElement types;

    public FaintLight(EntityType<?> entityTypeIn, Level worldIn) {
    ...
    public FaintLight(EntityType<?> entityTypeIn, Level worldIn, IElement type) {
        super(entityTypeIn, worldIn);
        this.types = type;
        double d1 = this.getX() + 0.5D;
        double d2 = this.getY() + 0.5D;
