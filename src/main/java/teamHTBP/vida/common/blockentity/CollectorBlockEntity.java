...
public class CollectorBlockEntity extends VidaBaseBlockEntity {
    //最大收集值
    ...
    public CollectorBlockEntity(BlockPos pWorldPosition, BlockState pBlockState) {
        super(VidaBlockEntityLoader.TileEntityCollector.get(), pWorldPosition, pBlockState);
    }
    ...
    public boolean setCore(ItemStack itemStack) {
        if (itemStack.getItem() == VidaItemLoader.ELEMENTCORE_VOID.get()) {
    ...
    //是否放入了空核心
    public boolean hasEmptyElementCore() {return coreItem != null && coreItem.getItem() == VidaItemLoader.ELEMENTCORE_VOID.get();}
    ...
    public void generateCompleteCore(){
        if (element instanceof EnumElements) {
            switch ((EnumElements)element) {
                case GOLD:
                    this.coreItem = new ItemStack(VidaItemLoader.ELEMENTCORE_GOLD.get(), 1);
                    break;
                case WOOD:
                    this.coreItem = new ItemStack(VidaItemLoader.ELEMENTCORE_WOOD.get(), 1);
                    break;
                case AQUA:
                    this.coreItem = new ItemStack(VidaItemLoader.ELEMENTCORE_AQUA.get(), 1);
                    break;
                case FIRE:
                    this.coreItem = new ItemStack(VidaItemLoader.ELEMENTCORE_FIRE.get(), 1);
                    break;
                default:
                    this.coreItem = new ItemStack(VidaItemLoader.ELEMENTCORE_EARTH.get(), 1);
                    break;
            }
        }
    }
