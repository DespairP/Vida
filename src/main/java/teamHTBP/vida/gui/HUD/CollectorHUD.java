...
public class CollectorHUD extends AbstractGui {
    ...
    private final ResourceLocation HUD = new ResourceLocation(Vida.modId, "textures/gui/collector_hud.png");
    ...
    public void render(){
        ...
        blit(screenWidth + 2 , screenHeight + 30, 0, 6, 29, 20, 3, 48, 48);
        blit(screenWidth + 4 , screenHeight + 40, 0, 8, 33, 15, 13, 48, 48);
        blit(screenWidth, screenHeight, 0, 4, 4, 24, 24, 48, 48);
        int progress = (int) (10 * tileEntityCollector.getCollection() / tileEntityCollector.MAX_COLLECTION);
        ...
        if(tileEntityCollector.isCollect){
            blit(screenWidth + 2 , screenHeight + 30, 0, 26, 29, progress, 3, 48, 48);
            blit(screenWidth + 2 + 20 - progress, screenHeight + 30 , 0, 26 + 20 - progress, 29 , progress, 3, 48, 48);
        }
        if(tileEntityCollector.isCollect && tileEntityCollector.getCollection() == 0 ){
            blit(screenWidth + 20 , screenHeight + 20, 0, 31, 5, 13, 13, 48, 48);
        }else if(tileEntityCollector.isCollect){
            blit(screenWidth + 20 , screenHeight + 20, 0, 31, 19, 13, 10, 48, 48);
        }
        drawString(minecraft.fontRenderer,  progressPercent + "%",screenWidth + 7,screenHeight - 10,60000);
