...
public class NormalSkillSlot extends AbstractSkillSlot {
    ...
    public NormalSkillSlot(int xIn, int yIn, int widthIn, int heightIn, String msg,int index) {
        super(xIn, yIn, widthIn, heightIn, msg);
        this.index = index;
    }

    public NormalSkillSlot(int xIn, int yIn, int widthIn, int heightIn, String msg, int index, Skill skill, ContainerScreenInjectTable containerScreen) {
    ...
    public void renderSkillFrame(){
        ...
        if(!this.isComplete){
            if(!this.getMouseFocus()){
                ...
                this.blit(this.x, this.y, 0,76,192, 38,38, 512, 512);}
            else{
                RenderSystem.color4f(1, 1, 1, this.alpha + this.hoveredLight + this.focusAlpha - overlapAlpha);
                this.blit(this.x, this.y, 0,76,260, 38,38, 512, 512);}
        }else{
            if(!this.getMouseFocus()){
