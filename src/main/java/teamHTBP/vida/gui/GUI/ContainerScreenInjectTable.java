...
public class ContainerScreenInjectTable extends ContainerScreen<ContainerInjectTable> {
    ...
    private void initSkillAndSkillSlot(){
        ...
        skills.forEach((name,skill)->{
            ...
            slot.listener = (id,in,skillSlot) ->{if(buttonClickIndex == -1 || buttonClickIndex != in){buttonClickIndex = in; buttonClickSkillName = name;}
            else {buttonClickIndex = -1; buttonClickSkillName = "";}
                this.skillSlots.forEach(skillSlots->{ skillSlots.setMouseFouse(false);
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        alpha += 0.1f;     if(alpha >= 1.0f) alpha = 1.0f;
        refreshTick = (refreshTick + 1) % MAX_REFRESH_TIME;
        CompoundNBT nbt = stack.getOrCreateTag();
        drawForegGround(); //绘制背景
        drawSkillIntroducution(); //绘制技能介绍
        drawSkill(); //调整技能位置
        drawLevel(nbt,this.stack); //绘制工具等级
        drawSkillEnergy();
    ...
    private void drawSword() {
        ...
        //渲染物品的外框
        this.minecraft.getTextureManager().bindTexture(Gui);
        blit(this.backgroundXsize - 21 + offsetX, this.backgroundYsize - 24 + offsetY,0,91,56,42, 48,512,512);
    ...
    public void render(int mouseX, int mouseY, float ticks) {
        ...
        renderHoveredToolTip(mouseX, mouseY); //渲染原生的tooltip
        skillSlots.forEach((slot)->{if(slot.isHovered()) this.renderSkillToolTip(mouseX,mouseY,slot.getSkillSurface());}); //当悬浮技能框上时会显示技能名字
    }
    ...
    /**画左上角的等级框**/
    public void drawLevel(CompoundNBT nbt,ItemStack stack){
        ...
        //渲染进度条
        blit(this.guiLeft + 14 + backgroundXsize / 20, this.guiTop + 9 + 10, 0, 70,238 ,progressLevel,2,512,512);
        //渲染左侧元素所属区域
        renderGem(stack);
        RenderSystem.popMatrix();
    }
    ...
    /**元素渲染*/
    private void renderGem(ItemStack itemStack){
        Item itemTool = stack.getItem();
        ...
        if(itemTool instanceof IElementTools){
            ...
            switch (elementType){
                case GOLD:
                    blit(this.guiLeft + backgroundXsize / 20, this.guiTop + 10,0,39,241,13,13,512,512);
                    break;
                case WOOD:
                    blit(this.guiLeft + backgroundXsize / 20, this.guiTop + 10,0,26,241,13,13,512,512);
                    break;
                case AQUA:
                    blit(this.guiLeft + backgroundXsize / 20, this.guiTop + 10,0,13,241,13,13,512,512);
                    break;
                case FIRE:
                    blit(this.guiLeft + backgroundXsize / 20, this.guiTop + 10,0,0,241,13,13,512,512);
                    break;
                case EARTH:
                    blit(this.guiLeft + backgroundXsize / 20, this.guiTop + 10,0,52,241,13,13,512,512);
                    break;
            }
        }
    }
    ...
    public void drawSkillEnergy(){
        ...
        this.minecraft.getTextureManager().bindTexture(Gui);
        //绘制原框
        blit(backgroundXsize - 55,this.guiTop + backgroundYsize * 2 - 25,0,175,150, 111 ,7 ,512,512);
        //绘制低点位
        blit(backgroundXsize - 54, this.guiTop + backgroundYsize * 2 - 23, 0, 176,159,1,3,512,512);
        //绘制高点位
        blit(backgroundXsize - 52 + selectSkillProgressLength, this.guiTop + backgroundYsize * 2 - 23,0,176,159,1,3,512,512);
        //绘制条
        blit(backgroundXsize - 53,this.guiTop+backgroundYsize * 2 - 24, 0, 177,158,selectSkillProgressLength + 1, 5, 512,512);
        RenderSystem.popMatrix();
    }

    public void drawSkillIntroducution(){
        drawBoard();
        if(this.buttonClickIndex > -1){
            ...
            //绘制文字
            drawCenteredString(this.font, surface.getSkillTranlateName(),this.guiLeft + this.xSize - 54 -  1 * this.backgroundXsize / 50,this.guiTop + 1 * this.backgroundYsize / 3, Integer.parseInt(selectedSlot.isLock()?"FF3C00":"45E800",16));
            //绘制介绍
            if(selectedSlot.isLock()){
                drawCenteredString(this.font,"???",this.guiLeft + this.xSize - 54  -  1 * this.backgroundXsize / 50,this.guiTop + 3 * this.backgroundYsize / 7,Integer.parseInt("C0C0C0",16));
                //绘制需要的前置技能
                drawCenteredString(this.font,I18n.format("skill.vida.requiredSkill.anno"),this.guiLeft + this.xSize - 54  -  1 * this.backgroundXsize / 50,this.guiTop + 6 * this.backgroundYsize / 7,Integer.parseInt("FF6A00",16));
                AtomicInteger integer = new AtomicInteger(0);
                //获取所有的前置技能
                surface.dependentSkill.forEach((id)->{
                    //从现有的skill中获取然后显示
                    ISkill skill = this.skills.get(id);
                    if(skill != null) drawCenteredString(this.font,"·"+skill.getSkillTranlateName(),this.guiLeft + this.xSize - 54  -  1 * this.backgroundXsize / 50,this.guiTop + 10 + 10 * integer.getAndIncrement() +6 * this.backgroundYsize / 7,Integer.parseInt("FF615F",16));
                });
            }else{
                //解锁后绘制解锁后的介绍文字
                this.font.drawSplitString(surface.getSkillTranlateDesc(),this.guiLeft + this.xSize - 104  -  1 * this.backgroundXsize / 50,this.guiTop  + 3 * this.backgroundYsize / 7,98,Integer.parseInt("90E8E8",16));
            }
            RenderSystem.popMatrix();
        }
        this.minecraft.getTextureManager().bindTexture(Gui);
    ...
    public void drawBoard(){
        ...
        blit(this.guiLeft + this.xSize - 114 -   1 * this.backgroundXsize / 100,this.guiTop + 1 * this.backgroundYsize / 4,0,173,5,114,142,512,512 );
        RenderSystem.popMatrix();
    }

    /**渲染技能的提示悬浮框**/
    public void renderSkillToolTip(int mouseX,int mouseY,SkillSurface skill){
    ...
    public void drawForegGround(){
        ...
        blit(0 + offsetX/10, 0 + offsetY/10, 0, 0, 0, 64, 64, 64, 64);
        RenderSystem.popMatrix();
        this.minecraft.getTextureManager().bindTexture(Gui);
    }
    ...
    public boolean isSkillLocked(SkillSurface surface){
        if(surface.dependentSkill == null || surface.dependentSkill.size() == 0) return false;
            ...
            else if(dependSkill.skillCurrentLevel == dependSkill.maxSkillLevel && dependSkill.skillCurrentExp == dependSkill.getRequiredExp(dependSkill.maxSkillLevel)) return false;
            else return true;
        }
        return false;
    }
