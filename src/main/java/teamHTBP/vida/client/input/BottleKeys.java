...
public class BottleKeys {
    ...
    public static void onKeyboardInput(InputEvent.KeyInputEvent event) {
        ...
        if (MESSAGE_KEY_1.consumeClick()) {
            VidaPacketManager.INSTANCE.sendToServer(new BottlesPacket(1));
        }
        if (MESSAGE_KEY_2.consumeClick()) {
            VidaPacketManager.INSTANCE.sendToServer(new BottlesPacket(2));
        }
        if (MESSAGE_KEY_3.consumeClick()) {
            VidaPacketManager.INSTANCE.sendToServer(new BottlesPacket(3));
        }
    }
