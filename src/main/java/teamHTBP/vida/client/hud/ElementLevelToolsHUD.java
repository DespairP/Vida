...
import teamHTBP.vida.client.event.listener.hud.HUDElementItemHandler;
...
public class ElementLevelToolsHUD extends GuiComponent {
    ...
    public void render(PoseStack matrixStack, float partialTicks){
        ...
        switch (element) {
            case GOLD -> blit(matrixStack, renderX, renderY, 0, 0, 0, 16, 16, 35, 119);
            case WOOD -> blit(matrixStack, renderX, renderY, 0, 48, 0, 16, 16, 35, 119);
            case AQUA -> blit(matrixStack, renderX, renderY, 0, 32, 0, 16, 16, 35, 119);
            case FIRE -> blit(matrixStack, renderX, renderY, 0, 16, 0, 16, 16, 35, 119);
            case EARTH -> blit(matrixStack, renderX, renderY, 0, 64, 0, 16, 16, 35, 119);
        }
        // draw progress
        switch (element) {
            case GOLD -> blit(matrixStack, renderX, renderY + 16 - progressHeight, 0, 0, 32 - progressHeight, 16, progressHeight, 35, 119);
            case WOOD -> blit(matrixStack, renderX, renderY + 16 - progressHeight, 0, 48, 32 - progressHeight, 16, progressHeight, 35, 119);
            case AQUA -> blit(matrixStack, renderX, renderY + 16 - progressHeight, 0, 32, 32 - progressHeight, 16, progressHeight, 35, 119);
            case FIRE -> blit(matrixStack, renderX, renderY + 16 - progressHeight, 0, 16, 32 - progressHeight, 16, progressHeight, 35, 119);
            case EARTH -> blit(matrixStack, renderX, renderY + 16 - progressHeight, 0, 64, 32 - progressHeight, 16, progressHeight, 35, 119);
        }
        matrixStack.popPose();
        matrixStack.pushPose();
        // 显示工具等级
