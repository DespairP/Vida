...
public class BlockItemAutoRegister {
    /**储存需要注册的方块*/
    ...
    /**
     * 对方块进行BlockItem的注册
     * 分别调用{@link BlockItemAutoRegister#init()}进行需要注册的Block的获取，
     * {@link BlockItemAutoRegister#inject()}进行Item的注入
     * */
    @SubscribeEvent
    public static void registerBlockItems(RegistryEvent.Register<Item> event) throws IllegalAccessException {
        ...
        REGISTRY_BLOCK_LIST.forEach((key,block) -> {
            Item.Properties properties = new Item.Properties().tab(ItemGroupRegistry.vidaItemGroup);
    ...
    private static void init() throws IllegalAccessException {
        for(Field decoratedBlock : VidaBlockRegistry.class.getDeclaredFields()){
    ...
    private static void inject() throws IllegalAccessException {
        for(Field itemField : VidaItemRegistry.class.getDeclaredFields()){
