    ...
    public static void onEvent(EntityRenderersEvent.RegisterLayerDefinitions event) {
        try {
            ...
            for (ModelData modelData : dataList) {
                String to = CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, modelData.getClass().getSimpleName());
                ModelLayerLocation name = register(to);

                FACTORY.put(modelData.aClass, () -> {
                    try {
                        return (Model) modelData.aClass.getConstructor(ModelPart.class)
                ...
                Method createBodyLayer = modelData.aClass.getMethod("createBodyLayer");
